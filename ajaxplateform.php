...
$searchQuery = " ";
if ($searchValue != '') {
	$searchQuery = " and (crg.plateform_name like '%" . $searchValue . "%')";
}
...
if ($_SESSION['role'] == 1) {
	...
	$queuenames = "SELECT crg.id AS id FROM plateform as crg";
...
if ($_SESSION['role'] == 1) {
	$sel1 = "select count(*) as allcount from plateform " . $queuenamesid . "";
} else {
	$sel1 = "select count(*) as allcount from plateform " . $queuenamesid . "";
}
// echo $sel1;exit;
...
if ($_SESSION['role'] == 1) {
	$sel2 = "select count(*) as allcount from plateform crg WHERE 1 " . $queuenamesidss . " " . $searchQuery;
} else {
	$sel2 = "select count(*) as allcount from plateform crg WHERE " . $queuenamesidss . " " . $searchQuery;
}
...
## Fetch records
if ($_SESSION['role'] == 1) {
	$empQuery = "select crg.* from plateform crg WHERE 1 " . $queuenamesidss . " " . $searchQuery . " order by " . $columnName . " " . $columnSortOrder . " limit " . $row . "," . $rowperpage;
} else {
	$empQuery = "select crg.* from plateform crg WHERE " . $queuenamesidss . " " . $searchQuery . " order by " . $columnName . " " . $columnSortOrder . " limit " . $row . "," . $rowperpage;
}
// echo $empQuery;exit;
...
while ($row = mysqli_fetch_assoc($empRecords)) {
	...
	$data[] = array(
		...
		"action" => '<a href="plateformedit.php?id=' . $row['id'] . '">
<button class="item btn btn-sm btn-outline-success"><i class="fa fa-pencil"></i></button></a>
<a href="javascript:void(0)" onclick="return deletePlateform(' . $row['id'] . ');" type="button" class=""><button class="item btn btn-sm btn-outline-danger"><i class="fa fa-trash-o"></i></button></a>
'
	);
	$i++;
}
