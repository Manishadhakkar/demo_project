...
$searchQuery = " ";
if ($searchValue != '') {
	$searchQuery = " and (crg.name like '%" . $searchValue . "%' or 
...
if ($_SESSION['role'] == 1) {
	...
	$queuenames = "SELECT crg.id AS id FROM users as crg";
...
if ($_SESSION['role'] == 1) {
	$sel1 = "select count(*) as allcount from users " . $queuenamesid . "";
} else {
	$sel1 = "select count(*) as allcount from users " . $queuenamesid . "";
}
// echo $sel1;exit;
...
if ($_SESSION['role'] == 1) {
	$sel2 = "select count(*) as allcount from users crg WHERE 1 " . $queuenamesidss . " " . $searchQuery;
} else {
	$sel2 = "select count(*) as allcount from users crg WHERE " . $queuenamesidss . " " . $searchQuery;
}
...
## Fetch records
if ($_SESSION['role'] == 1) {
	$empQuery = "select crg.* from users crg WHERE 1 " . $queuenamesidss . " " . $searchQuery . " order by " . $columnName . " " . $columnSortOrder . " limit " . $row . "," . $rowperpage;
} else {
	$empQuery = "select crg.* from users crg WHERE " . $queuenamesidss . " " . $searchQuery . " order by " . $columnName . " " . $columnSortOrder . " limit " . $row . "," . $rowperpage;
}
// echo $empQuery;exit;
...
while ($row = mysqli_fetch_assoc($empRecords)) {
	...
	if ($row['role'] == '1') {
		...
		$role = "User";
		$action = '<a href="useredit.php?id=' . $row['id'] . '">
