...
$searchQuery = " ";
if ($searchValue != '') {
	$searchQuery = " and (crg.w_name like '%" . $searchValue . "%' or 
         crg.w_code like '%" . $searchValue . "%')";
}
...
if ($_SESSION['role'] == 1) {
	...
	$queuenames = "SELECT crg.id AS id FROM warehouse as crg";
...
if ($_SESSION['role'] == 1) {
	$sel1 = "select count(*) as allcount from warehouse " . $queuenamesid . "";
} else {
	$sel1 = "select count(*) as allcount from warehouse " . $queuenamesid . "";
}
// echo $sel1;exit;
...
if ($_SESSION['role'] == 1) {
	$sel2 = "select count(*) as allcount from warehouse crg WHERE 1 " . $queuenamesidss . " " . $searchQuery;
} else {
	$sel2 = "select count(*) as allcount from warehouse crg WHERE " . $queuenamesidss . " " . $searchQuery;
}
...
## Fetch records
if ($_SESSION['role'] == 1) {
	$empQuery = "select crg.* from warehouse crg WHERE 1 " . $queuenamesidss . " " . $searchQuery . " order by " . $columnName . " " . $columnSortOrder . " limit " . $row . "," . $rowperpage;
} else {
	$empQuery = "select crg.* from warehouse crg WHERE " . $queuenamesidss . " " . $searchQuery . " order by " . $columnName . " " . $columnSortOrder . " limit " . $row . "," . $rowperpage;
}
// echo $empQuery;exit;
...
while ($row = mysqli_fetch_assoc($empRecords)) {
	...
	$data[] = array(
		...
		"action" => '<a href="warehouse_edit.php?id=' . $row['id'] . '">
